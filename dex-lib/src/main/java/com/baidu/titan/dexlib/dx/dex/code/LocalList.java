...
import com.baidu.titan.dexlib.dx.util.FixedSizeList;
...
public final class LocalList extends FixedSizeList {
    ...
    public LocalEntry getLocalEntry(int n) {
        return (LocalEntry) get0(n);
    }
     ...
     * @param entry {@code non-null;} the entry to set at {@code n}
     */
    public void setLocalEntry(int n, LocalEntry entry) {
        set0(n, entry);
    }
    ...
    public static class LocalEntry implements Comparable<LocalEntry>{
        public int reg;
        public String name;
        public String type;
        public String signature;
        public CodeAddress start;
        public CodeAddress end;
        ...
        public LocalEntry(int reg, String name, String type, String signature, CodeAddress start, CodeAddress end) {
            this.reg = reg;
            this.name = name;
            this.type = type;
        ...
        public boolean canRestart(LocalEntry other) {
        ...
        @Override
        public int compareTo(LocalEntry other) {
            int startCompare = this.start.getAddress() - other.start.getAddress();
            if (startCompare != 0) {
                return startCompare;
            }

            int regCompare = this.reg - other.reg;
            if (regCompare != 0) {
                return regCompare;
            }

            int endCompare = this.end.getAddress() - other.end.getAddress();
            if (endCompare != 0) {
                return -endCompare;
            }
    ...
    public static LocalList make(List<LocalEntry> entries) {
        if (entries == null || entries.size() == 0) {
            return new LocalList(0);
        }
        ArrayList<LocalEntry> sorted = new ArrayList<>(entries);
        Collections.sort(sorted);
        LocalList resultList = new LocalList(sorted.size());
        for (int i = 0; i < sorted.size(); i++) {
            resultList.setLocalEntry(i, sorted.get(i));
        }
