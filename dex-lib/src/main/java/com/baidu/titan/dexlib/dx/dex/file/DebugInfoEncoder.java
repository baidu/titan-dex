...
package com.baidu.titan.dexlib.dx.dex.file;

import com.baidu.titan.dexlib.dex.util.ExceptionWithContext;
import com.baidu.titan.dexlib.dx.dex.code.LocalList;
import com.baidu.titan.dexlib.dx.dex.code.PositionList;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_ADVANCE_LINE;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_ADVANCE_PC;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_END_LOCAL;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_END_SEQUENCE;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_FIRST_SPECIAL;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_LINE_BASE;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_LINE_RANGE;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_RESTART_LOCAL;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_SET_PROLOGUE_END;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_START_LOCAL;
import static com.baidu.titan.dexlib.dx.dex.file.DebugInfoConstants.DBG_START_LOCAL_EXTENDED;
...
import com.baidu.titan.dexlib.dx.rop.type.Prototype;
...
import com.baidu.titan.dexlib.dx.util.ByteArrayAnnotatedOutput;
 ...
 * for each method argument in left-to-right orderencod
...
public final class DebugInfoEncoder {
    ...
    private final LocalList.LocalEntry[] lastLocalEntryForReg;
    ...
    public DebugInfoEncoder(PositionList positions, LocalList locals,
            ...
            boolean isStatic, CstMethodRef ref, String[] parameterNames) {
        ...
        lastLocalEntryForReg = new LocalList.LocalEntry[regSize];
        mParaNames = parameterNames;
    }
...
//        ArrayList<LocalList.Entry> methodArgs = extractMethodArguments();
        ...
        emitHeader(sortedPositions, mParaNames);
...
//        int localsSz = locals.size();
        ...
        for (;;) {
            ...
            int nextAddrL = emitLocals();
                    //Integer.MAX_VALUE; // local variable
...
//            }
    ...
    private int emitLocals() throws IOException {
        int sz = locals.size();
        int delta = Integer.MAX_VALUE;
        for (int i = 0; i < sz; i++) {
            LocalList.LocalEntry localEntry = locals.getLocalEntry(i);
            if (localEntry.status == LocalList.LocalEntry.STATUS_END) {
                continue;
            }

            int reg = localEntry.reg;
            int startAddress = localEntry.start.getAddress();
            int endAddress = localEntry.end.getAddress();
            int startDelta = startAddress - address;
            int endDelta = endAddress - address;
            LocalList.LocalEntry prevLocal = lastLocalEntryForReg[reg];

            if (startDelta == 0) {
                localEntry.status = LocalList.LocalEntry.STATUS_START;
                if (prevLocal != null && localEntry.canRestart(prevLocal)) {
                    emitLocalRestart(localEntry);
                } else {
                    emitLocalStart(localEntry);
                }
                lastLocalEntryForReg[reg] = localEntry;
                delta = Math.min(endDelta, delta);
                continue;
            }

            if (endDelta == 0) {
                emitLocalEnd(localEntry);
                localEntry.status = LocalList.LocalEntry.STATUS_END;
                continue;
            }

            if (localEntry.status == LocalList.LocalEntry.STATUS_START) {
                delta = Math.min(delta, endDelta);
            } else {
                delta = Math.min(delta, Math.min(startDelta, endDelta));
            }
        }
        return (delta == Integer.MAX_VALUE) ? Integer.MAX_VALUE : delta + address;
    }
    ...
    private void emitHeader(ArrayList<PositionList.Entry> sortedPositions,
                            String[] paraNames) throws IOException {
        boolean annotate = (annotateTo != null) || (debugPrint != null);
        int mark = output.getCursor();
...
//        }
        ...
        for (int i = 0; i < szParamTypes; i++) {
            Type pt = paramTypes.get(i);
            String name = (paraNames != null && paraNames.length > i) ? paraNames[i] : null;
            if (name == null) {
                emitStringIndex(null);
            } else {
                emitStringIndex(new CstString(name));
            }
...
//        }
    ...
    private void emitLocalRestart(LocalList.LocalEntry entry)
            throws IOException {

        int mark = output.getCursor();

        output.writeByte(DBG_RESTART_LOCAL);
        emitUnsignedLeb128(entry.reg);
...
//        }
    ...
    private void emitLocalStart(LocalList.LocalEntry entry)
            throws IOException {
        ...
        emitUnsignedLeb128(entry.reg);
        emitStringIndex(new CstString(entry.name));
        emitTypeIndex(CstType.intern(Type.intern(entry.type)));
...
//        }
    ...
    private void emitLocalStartExtended(LocalList.LocalEntry entry)
            throws IOException {
        ...
        emitStringIndex(new CstString(entry.name));
        emitTypeIndex(CstType.intern(Type.intern(entry.type)));
        emitStringIndex(new CstString(entry.signature));
...
//        }
    ...
    private void emitLocalEnd(LocalList.LocalEntry entry)
            throws IOException {

        int mark = output.getCursor();

        output.writeByte(DBG_END_LOCAL);
        output.writeUleb128(entry.reg);
//
//        if (annotateTo != null || debugPrint != null) {
//            annotate(output.getCursor() - mark,
//                    String.format("%04x: -local %s", address,
...
//        }
    ...
    private void emitPosition(PositionList.Entry entry)
        ...
        for (int newLine : pos.getLines()) {
            emitPosition(entry.getAddress(), newLine);
        }
    }
     ...
     *
     * @param newAddress position entry to emit.
     * @throws IOException
     */
    private void emitPosition(int newAddress, int newLine)
            throws IOException {
