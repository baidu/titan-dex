...
import com.baidu.titan.dexlib.dx.dex.code.DalvInsnList;
...
public final class CodeItem extends OffsettedItem {
    ...
    public void addContents(DexFile file) {
        ...
        if (code.getParameterNames() != null) {
            for (String pn: code.getParameterNames()) {
                if (pn != null) {
                    file.internIfAppropriate(new CstString(pn));
                }
            }
        }

        List<LocalList.LocalEntry> ll = code.getLocalEnties();
        if (ll != null) {
            for (LocalList.LocalEntry le: ll) {
                if (le.name != null) {
                    file.internIfAppropriate(new CstString(le.name));
                }
                if (le.signature != null) {
                    file.internIfAppropriate(new CstString(le.signature));
                }
                if (le.type != null) {
                    file.internIfAppropriate(new CstType(Type.intern(le.type)));
                }
    ...
    protected void place0(Section addedTo, int offset) {
        ...
        code.assignIndices(new DalvCode.AssignIndicesCallback() {
            ...
            public int getCurrentMethodId() {
                IndexedItem methodIdItem = file.findItemOrNull(ref);
            ...
            public int getCurrentClassId() {
                IndexedItem classIdItem = file.findItemOrNull(ref.getDefiningClass());
                if (classIdItem == null) {
                    return -1;
                }
                return classIdItem.getIndex();
            }
        });
